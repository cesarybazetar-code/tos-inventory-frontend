...
function CatalogEditor() {
  ...
  // Fixed lists (edit to match your restaurant)
  const LOCATIONS = [
    'Cooking Line','Meat','Seafood','Dairy','Produce','Dry & Other','Freezer','Bev & Coffee','Grocery'
  ];
  ...
  const load = async () => {
    ...
    if (locFilter.trim()) qs.set('area', locFilter.trim());
    const data = await apiGet('/items' + (qs.toString() ? `?${qs.toString()}` : ''));
    setItems(data);
  };
  React.useEffect(() => { load(); }, [search, locFilter]);
  ...
  // Display-only: same logic as backend for previewing per-unit
  ...
  return (
    <div className="card">
      ...
      <div className="muted screen-only" style={{marginTop:-4, marginBottom:4}}>
        Set <b>Price basis</b> + either <b>Case size</b> or <b>Conversion</b> with an <b>Order unit price</b>.
        The per-unit price preview uses the same rules as the backend.
      </div>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', marginTop:10, minWidth:980 }}>
          <thead>
            <tr>
              ...
              <th style={{width:120}}>Per-Unit (calc)</th>
            ...
            {items.map(it => {
              ...
              return (
                <tr key={it.id}>
                  ...
                  <td><span className="muted">{pu ?? '-'}</span></td>
