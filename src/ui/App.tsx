...
async function apiGet(path:string){ const base = localStorage.getItem('VITE_API_BASE_URL') || API_DEFAULT; const r = await fetch(base + path); return r.json(); }
...
function Importer(){
  ...
  const upload = async (file:File)=>{
    ...
    setMsg(`Imported ${data.created} items`);
  }
...
function Items(){
  const [items,setItems]=useState<Item[]>([]); const [name,setName]=useState(''); const [area,setArea]=useState('Cooking Line'); const [par,setPar]=useState(0);
  const load = async()=> setItems(await apiGet('/items')); useEffect(()=>{load();},[]);
  const add = async()=>{ if(!name.trim()) return; await apiPost('/items', { name, storage_area:area, par }); setName(''); setPar(0); load(); }
  return (<div className="card"><h3>Items</h3>
    ...
    </select><button className="btn" onClick={add}>Add</button></div>
    {items.map(i=>(<div key={i.id} className="card small"><b>{i.name}</b> — <span className="muted">{i.storage_area||'-'}</span> | PAR: {i.par||0}</div>))}
  </div>)
}

function Counts(){
  const [items,setItems]=useState<Item[]>([]); const [area,setArea]=useState('Cooking Line'); const [lines,setLines]=useState<Record<number,number>>({}); const [recent,setRecent]=useState<Count[]>([]);
  const load = async()=>{ setItems(await apiGet('/items')); setRecent(await apiGet('/counts')); }; useEffect(()=>{load();},[]);
  const save = async()=>{
  ...
  return (<div className="card"><h3>Counts</h3>
    ...
    {areaItems.map(i=>(<div key={i.id} className="row"><div>{i.name}</div>
      <input type="number" value={lines[i.id] || ''} onChange={e=>setLines(prev=>({...prev,[i.id]:parseFloat(e.target.value||'0')}))} /></div>))}
    <div className="muted">{recent.length} count session(s)</div></div>)
}
